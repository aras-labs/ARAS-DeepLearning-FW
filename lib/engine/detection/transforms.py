...
class Color(nn.Module):
    def forward(self, image, target):
        if random.random() > .2:
            image = F.gaussian_blur(image, kernel_size=[5, 9], sigma=[0.1, 2])
        # if random.random() > .3:
        #     image = F.posterize(image, bits=random.randint(3, 6))
        # if random.random() > .5:
        #     image = F.autocontrast(image)
        if random.random() > .5:
            image = F.equalize(image)
        if random.random() > .2:
            image = F.adjust_sharpness(image, sharpness_factor=random.uniform(.5, 1.5))
...
class PreProcess(nn.Module):
    def forward(self, image, target):
        # image = F.gaussian_blur(image, kernel_size=[5, 9], sigma=[0.1, 5])
        # image = F.posterize(image, bits=random.randint(3, 6))
        image = F.autocontrast(image)
        # image = F.adjust_contrast(image, contrast_factor=1.5)
        # image = F.equalize(image)
        image = F.adjust_sharpness(image, sharpness_factor=5)
        return image, target
